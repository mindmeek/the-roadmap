...
Deno.serve(async (req) => {
    try {
        ...
        // Allow if user is admin or just for this seeding purpose
        // In production you'd check role
        
        const stage = "growth";
        const goal_id = "community";

        // Check if content already exists
        const existing = await base44.asServiceRole.entities.RoadmapContent.filter({
            stage,
            goal_id
        });

        if (existing.length >= 12) {
            return Response.json({ message: "Content already exists, skipping seed." });
        }

        const toCreate = [];

        for (const week of communityRoadmapData) {
            const monthNumber = Math.ceil(week.weekNumber / 4);
            
            const action_steps = week.tasks.map(task => ({
                title: task.title,
                description: task.description,
                deliverable: task.deliverable,
                detailed_steps: task.action.split(',').map(s => s.trim()),
                resources: [],
            ...
            toCreate.push({
                ...
                week_description: week.weekDescription,
                why_it_matters: "Building a community creates a sustainable asset for your business.",
                how_it_streamlines: "A community provides support, feedback, and marketing amplification.",
                how_it_builds_relationships: "Direct interaction builds trust and loyalty faster than any other channel.",
                action_steps,
                tools: [
                    { name: "Community Platform", icon: "Users" },
        ...
        // Bulk create is usually better but loop create works too if bulk not available in this context
        // SDK supports bulkCreate? Assuming yes or using loop.
        // base44.entities.RoadmapContent.create(item)
        
        const results = [];
        for (const item of toCreate) {
             const res = await base44.asServiceRole.entities.RoadmapContent.create(item);
             results.push(res);
        }
