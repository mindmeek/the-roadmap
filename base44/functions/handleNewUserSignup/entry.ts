...
Deno.serve(async (req) => {
    try {
        ...
        try {
            await base44.asServiceRole.functions.invoke('createGoHighLevelContact', {
                email: email,
                fullName: fullName
            });
            console.log('GoHighLevel contact created for:', email);
        } catch (ghlError) {
            console.error('Error creating GoHighLevel contact:', ghlError);
            // Continue even if GHL fails - don't block user signup
        }

        return Response.json({
            success: true,
            message: 'User trial activated, welcome email sent, and GoHighLevel contact created',
            trial_expires_on: trialExpiryDate
        });
